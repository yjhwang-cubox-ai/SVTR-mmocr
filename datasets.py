...
class TNGODataset(Dataset):
    def __init__(self, json_path, character_dict_path="dict/vietnam_dict.txt"):
    ...
    def __getitem__(self, index):
        data = self._load_image_from_file(index)
        cv2.imwrite("1-before.jpg", data["img"])
        data = self.text_recog_general_aug(data)
        cv2.imwrite("2-after_aug.jpg", data["img"])
        data = self.crop_height(data)
        cv2.imwrite("3-after_crop.jpg", data["img"])
        data = self.gaussian_blur(data)
        cv2.imwrite("4-after_blur.jpg", data["img"])
        data = self.color_jitter(data)
        cv2.imwrite("5-after_jitter.jpg", data["img"])
        data = self.image_content_jitter(data)
        cv2.imwrite("6-after_content_jitter.jpg", data["img"])
        data = self.additive_gaussian_noise(data)
        cv2.imwrite("7-after_additive_noise.jpg", data["img"])
        data = self.reverse_pixels(data)
        cv2.imwrite("8-after_reverse_pixels.jpg", data["img"])
        data = self.resize(data)
        cv2.imwrite("9-after_resize.jpg", data["img"])
        return data
    
    def __len__(self):
        return len(self.data_list)
    
    def _load_image_from_file(self, index):
        img_path = os.path.join(self.dir_path, self.data_list[index]["img_path"])
        instances = self.data_list[index]["instances"]
