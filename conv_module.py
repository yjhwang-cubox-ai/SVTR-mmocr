...
def efficient_conv_bn_eval_forward(bn: nn.BatchNorm2d,
    ...
    return F.conv2d(x, weight_on_the_fly, bias_on_the_fly, conv.stride, conv.padding, conv.dilation, conv.groups)
...
def build_activation_layer(cfg):
    ...
    if act_type == 'ReLU':
        ...
        return nn.LeakyReLU(inplace=cfg.get('inplace', True))
    elif act_type == 'Tanh':
        return nn.Tanh()
    elif act_type == 'PReLU':
        return nn.PReLU()
...
def build_norm_layer(cfg: Dict,
    ...
    if norm_layer is not nn.GroupNorm:
        ...
        layer = norm_layer(num_groups=cfg_['num_groups'], num_channels=num_features, **cfg_)
...
class ConvModule(nn.Module):
    ...
    def turn_on_efficient_conv_bn_eval(self, efficient_conv_bn_eval=True):
        ...
        if efficient_conv_bn_eval and self.norm \
                            and isinstance(self.norm, nn.modules.batchnorm._BatchNorm) \
